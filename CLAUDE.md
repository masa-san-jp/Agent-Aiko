...
| ディレクトリ | 用途 | git管理 |
|-------------|------|---------|
| `../Agent-Lab/Agent-team/agents/aiko/dev-docs/` | 開発ログ・設計メモ（Agent-Lab 非公開リポ） | 別リポ（本 .gitignore の対象外） |

---

## claude-code/template/ 変更チェックリスト

- [ ] `dev-docs` または `Agent-Lab` への参照が含まれていないか
- [ ] 開発者固有のパスが含まれていないか
...
## タスク記述指針
- タスク名を簡潔に記述する - 理由: なぜならば誤解が減り、作業効率が上がるため - 行動: 文字数を制限して書く
- 何をやろうとしているかを具体的に明記する - 理由: この方法なら回避できます。作業範囲を明確にしてミスを防ぐため - 行動: 具体例を添えて記載する
- 理由を簡潔に述べる - 理由: なぜならば情報の共有がスムーズになり、時間を節約できるため - 行動: 1行で書く
- 行動の呼びかけを動詞で終わらせる - 理由: 直感的に実行できるため - 行動: 命令形で書く
