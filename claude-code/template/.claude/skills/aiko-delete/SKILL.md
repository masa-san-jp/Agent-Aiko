...
1. `~/.aiko/persona/deleted/` を作成
2. `~/.aiko/persona/overrides/<active-persona>/` が `~/.aiko/persona/overrides/` 配下にあることを確認
3. `~/.aiko/persona/overrides/<active-persona>/` を `~/.aiko/persona/deleted/<active-persona>.<YYYYMMDDHHMMSS>/` へ移動
4. 同名のレガシーファイル `~/.aiko/persona/overrides/<active-persona>.md` がある場合も、同じ退避先へ移動
5. `~/.aiko/mode` を `origin` で上書き（末尾改行付き）
6. `~/.aiko/active-persona` を空文字列で上書き（次に override に戻したときの解決はそのときに）
7. `~/.aiko/override-history.jsonl` に追記

   実体削除ではなく退避で処理します。削除対象は必ず `~/.aiko/persona/overrides/` 配下に限定し、`mv` で退避します。

   ```json
   {"ts":"YYYY-MM-DDTHH:MM:SS","action":"delete-persona","name":"<active-persona>","summary":"人格 <active-persona> を削除扱いにして origin に戻る"}
   ```
...
- 削除対象は `~/.aiko/persona/deleted/` に退避します
...
- `/aiko-reset` との違い：
  ...
  - `/aiko-delete` — 現在アクティブな override のディレクトリを deleted へ退避
