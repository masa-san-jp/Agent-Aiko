...
1. `~/.aiko/persona/overrides/<active-persona>/` を再帰削除（Bash `rm -rf`）
2. `~/.aiko/mode` を `origin` で上書き（末尾改行付き）
3. `~/.aiko/active-persona` を空文字列で上書き（次に override に戻したときの解決はそのときに）
4. `~/.aiko/override-history.jsonl` に追記
