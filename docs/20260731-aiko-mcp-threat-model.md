...
### T5. ローカルファイルからの情報漏洩（Medium・installer 側は対応済み 2026-07-31）
...
**対策と現状**
- ✅ installer が状態ファイル生成後に明示的にモードを設定する（claude-code / codex 両方）
- ✅ Fresh Install テストで、生成された各ファイルの**実モードを assert** する（§12.1 の Security Test）。
  この検査は、chmod を外すと実際に落ちることを確認してから入れている
- ✅ 既存インストールに対しては `aiko doctor --fix` が検出して直す
- ⬜ 置き場を決める（§11.3 は `~/.config/aiko/`、実装は `~/.aiko/`）。**未決**
