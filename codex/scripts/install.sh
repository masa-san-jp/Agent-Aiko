...
# 6/6 aiko shim
# ─────────────────────────────────────
# --aiko-home で既定以外へ入れたときは、その場所を shim に焼き込む。焼かないと
# 起動時に既定の ~/.aiko を見に行き、入れた場所を使えない（2026-07-31 実測）。
if [ "$AIKO_HOME" = "${HOME}/.aiko" ]; then
  SHIM_AIKO_HOME_LINE=""
else
  SHIM_AIKO_HOME_LINE="export AIKO_HOME=\"${AIKO_HOME}\"
"
fi
...
${SHIM_AIKO_HOME_LINE}exec node "${CODEX_DIR}/dist/aiko-shell.js" "\$@"
SHIM_EOF
chmod +x "$SHIM"
ok "${SHIM}"
