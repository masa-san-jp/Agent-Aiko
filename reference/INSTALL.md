...
`$REFERENCE_ROOT/skills/` 配下の全スキル（各 `<name>/` ディレクトリ）を `Agent-team/agents/.claude/skills/` にコピー。
必要なものだけ採用する場合は、コピーするディレクトリを絞ってください：

```bash
# skills/ 配下の全スキルディレクトリ（README.md 等の単体ファイルは除外）をコピー
for d in "$REFERENCE_ROOT"/skills/*/; do
  cp -r "$d" "$ORG_REPO_PATH/Agent-team/agents/.claude/skills/"
done
```
